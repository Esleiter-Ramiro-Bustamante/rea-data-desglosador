...
FLUJO COMPLETO v1.9:
  ...
  Advertencia de privacidad
    ↓
  Solicitar carpeta + nombre de archivo + mes del reporte
    ↓
  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
print("\n" + "─" * 70)
print("  PASO 1 — SELECCIÓN DEL ARCHIVO")
print("─" * 70)

# Carpeta configurable (no hardcodeada)
carpeta_default = os.path.join(
    os.path.expanduser('~'),
    'Desktop', 'GASTOS RESICO', '2026', 'FEBRERO26'
)
print(f"\n  Carpeta por defecto: {carpeta_default}")
carpeta_input = input("  Carpeta (Enter = usar la de arriba): ").strip()
desktop_path  = carpeta_input if carpeta_input else carpeta_default
...
file_name = input("\n  Nombre del archivo Excel (sin .xlsx): ").strip()
...
if not os.path.exists(file_path):
    ...
    if excels:
        ...
        for i, f in enumerate(excels[:10], 1):
            print(f"  {i}. {f}")
    input("\nENTER para salir...")
    raise SystemExit(1)

# Mes del reporte (para el título del HTML y Excel)
mes_reporte = input("\n  Mes del reporte (ej: FEBRERO 2026): ").strip()
if not mes_reporte:
    from datetime import datetime
    mes_reporte = datetime.now().strftime('%B %Y').upper()
